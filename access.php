...
<?php
	if (isset($_POST['submit']))
	{
		...
		//if (!empty($userid)&&!empty($admin)&&!empty($staff)&&!empty($doctor)&&!empty($admission)&&!empty($medical)&&!empty($stocks)&&!empty($bill)&&!empty($patient))
		//{
			$query = "SELECT `userid` FROM `access` WHERE `userid` = '$userid'";
				$query_run = mysql_query($query);
				
				if (mysql_num_rows($query_run)==1)
				{
					$message = '<p style=" color:red;">The Username already has permission in the database. Use the update option instead of this.</p>';
				}else
					{
	
						$con = "INSERT INTO access VALUES('$userid', '$admin', '$staff', '$doctor', '$admission', '$medical', '$stocks', '$bill', '$patient')";
						if (mysql_query($con))
						{
...
<div id="wrapper">
    ...
    <!-- End Slider --> 
	<h3 style="color: blue;">This is only supposed to be used for 1st time when granting access to users</h3>
	<h5 style="color: red;">Do not use this form for updating staff access</h5>
	<form class="jotform-form" action="access.php" method="post" name="form_32638191299464" id="32638191299464" accept-charset="utf-8">
