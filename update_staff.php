...
<?php
		if (isset($_POST['id'])&&isset($_POST['username'])&&isset($_POST['password'])&&isset($_POST['confirm_password'])&&isset($_POST['firstname'])&&isset($_POST['surname'])&&isset($_POST['email']))
		{
			$id = $_POST['id'];
			...
			if (!empty($username)&&!empty($password)&&!empty($confirm_password)&&!empty($firstname)&&!empty($surname)&&!empty($email))
			{
				...
				}else
				{
					...
					$query = "UPDATE `users` SET `username`='$username_hash',`password`='$password_hash',`firstname`='$firstname_hash',`surname`='$surname_hash',`email`='$email_hash' WHERE `id`='$id'";
					if ($query_run = mysql_query($query))
					{
						$message = '<center><p style="color: green;">Staff Record Updated Successfully</p></center>';
					}else
					{
						$message = '<center><p style="color: red;">Failed to Update</p></center>';
					}
...
<div id="wrapper">
	...
	<!-- Begin Content -->
	<div id="content">
	<h1 class="title">Update Staff Record</h1>
	<div class="line"></div>
	<?php echo $message; ?>
	<?php if (checkadmin()) { ?>	
	<div class="intro"></div>
	<!-- Form for registration -->
	<form class="jotform-form" action="update_staff.php" method="post" name="form_32743415307450" id="32743415307450" accept-charset="utf-8">
